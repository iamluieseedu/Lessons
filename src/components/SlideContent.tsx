...
export const SlideContent: React.FC<SlideContentProps> = ({ slide }) => {
  if (slide.type === 'cover') {
    return (
      <div className="h-full w-full flex flex-col justify-center items-start p-5 sm:p-8 md:p-14 relative z-10 text-slate-900 overflow-y-auto">
        {slide.moduleTag && (
          <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-sky-500/10 text-sky-700 border border-sky-500/20 text-xs sm:text-sm font-semibold mb-4 md:mb-6 shadow-sm">
        ...
        <p className="text-base sm:text-lg md:text-xl text-slate-600 max-w-3xl leading-relaxed mb-6 md:mb-10">
          {slide.subtitle}
        </p>
    ...
    return (
      <div className="h-full w-full flex flex-col justify-between p-4 sm:p-6 md:p-10 relative z-10 text-slate-900 overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 pb-2 md:pb-3 mb-3 md:mb-4">
          <h2 className="font-lexend text-lg md:text-2xl font-bold text-sky-700 flex items-center gap-1.5 md:gap-2">
              ...
              <div className="space-y-2.5 md:space-y-3">
                {slide.timelineItems?.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-3 sm:gap-4 p-2.5 sm:p-3 rounded-lg bg-slate-50 border border-slate-200/80">
                    <span className="font-lexend text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded bg-sky-500/10 text-sky-700 border border-sky-500/20 whitespace-nowrap mt-0.5">
                      {item.year}
                    </span>
                    <div>
                      <h4 className="font-semibold text-xs sm:text-sm text-slate-900">{item.title}</h4>
                      <p className="text-[11px] sm:text-xs text-slate-650 leading-relaxed">{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-stretch flex-grow">
          <div className="md:col-span-7 flex flex-col justify-between h-full">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-3 md:mb-4">
                {slide.versusLeft && (
                  <div className="p-3 md:p-4 rounded-xl bg-slate-50 border border-slate-200">
                    ...
                    <ul className="space-y-1 md:space-y-1.5 text-[11px] sm:text-xs text-slate-655 leading-relaxed">
                ...
                {slide.versusRight && (
                  <div className="p-3 md:p-4 rounded-xl bg-sky-500/[0.03] border border-sky-200">
                    <h4 className="font-semibold text-xs sm:text-sm text-sky-750 flex items-center gap-1.5 md:gap-2 mb-2">
              ...
              {slide.keyInsight && (
                <div className="p-3 md:p-3.5 rounded-lg bg-sky-500/[0.04] border-l-4 border-sky-500 text-slate-700">
                  <strong className="text-[10px] sm:text-xs uppercase tracking-wider text-sky-750 flex items-center gap-1.5 mb-1 font-lexend font-bold">
                    <Target className="w-3.5 h-3.5 md:w-4 md:h-4 text-sky-600" />
                    {slide.keyInsight.title || 'Key Insight'}
                  </strong>
                  <p className="text-[11px] sm:text-xs leading-relaxed text-slate-600">{slide.keyInsight.text}</p>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="h-full w-full flex flex-col justify-between p-4 sm:p-6 md:p-10 relative z-10 text-slate-900 overflow-y-auto">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-2 md:pb-3 mb-3 md:mb-4">
        <h2 className="font-lexend text-lg md:text-2xl font-bold text-sky-700 flex items-center gap-1.5 md:gap-2">
          <Film className="w-5 h-5 md:w-6 md:h-6 text-sky-600" />
          {slide.title}
        </h2>
        {slide.moduleTag && (
          <span className="px-2.5 py-0.5 md:px-3 md:py-1 rounded-full bg-sky-500/10 text-sky-700 border border-sky-500/20 text-[10px] md:text-xs font-semibold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-stretch flex-grow">
        ...
        <div className="md:col-span-7 flex flex-col justify-between h-full">
            ...
            {slide.bullets && slide.bullets.length > 0 && (
              <ul className="space-y-1.5 md:space-y-2 mb-3 md:mb-4">
                {slide.bullets.map((bullet, idx) => (
                  <li key={idx} className="text-xs sm:text-sm md:text-base text-slate-650 leading-relaxed flex items-start gap-2">
          ...
          <div className="mt-auto space-y-2.5 md:space-y-3 pt-2">
            {slide.layman && (
              <div className="p-3 md:p-3.5 rounded-r-lg bg-amber-500/[0.04] border-l-4 border-amber-500 text-slate-700 shadow-sm">
                <strong className="text-[10px] sm:text-xs uppercase tracking-wider text-amber-700 flex items-center gap-1.5 mb-1 font-lexend font-bold">
                  <Lightbulb className="w-3.5 h-3.5 md:w-4 md:h-4 text-amber-655" />
            ...
            {slide.visualTrick && (
              <div className="p-3 md:p-3.5 rounded-r-lg bg-emerald-500/[0.04] border-l-4 border-emerald-500 text-slate-700 shadow-sm">
            ...
            {slide.keyInsight && (
              <div className="p-3 md:p-3.5 rounded-r-lg bg-sky-500/[0.04] border-l-4 border-sky-500 text-slate-700 shadow-sm">
                <strong className="text-[10px] sm:text-xs uppercase tracking-wider text-sky-700 flex items-center gap-1.5 mb-1 font-lexend font-bold">
                  <Target className="w-3.5 h-3.5 md:w-4 md:h-4 text-sky-600" />
                  {slide.keyInsight.title || 'Key Insight'}
                </strong>
